...
def set_largeur_mat(l: int):
    """Sets the value of largeur_mat"""
    global largeur_mat
    largeur_mat = l


def set_civil_count(nb: int):
    """Sets the value of largeur_mat"""
    global civil_count
    civil_count = nb


def set_guard_count(nb: int):
    """Sets the value of largeur_mat"""
    global guard_count
    guard_count = nb


def generateUnknownCells():
...
def checkCivil(position: Tuple[int, int], content: HC):
...
def explore(
    ...
    while None in known_cells.values():
        ...
        if known_cells[position] == None:
            known_cells[position] = HC.EMPTY
        ...
        if None not in known_cells.values():
            break
        ...
        if len(path) == 0 or known_cells[path[len(path) - 1]] != None:
            ...
            not_visited_tmp = [
                cell
                for cell in known_cells.keys()
                if known_cells[cell] == None and cell != position
            ]
            print(known_cells)
            not_visited = []
            ...
            for cell in neighbours:
                if known_cells[cell] == None:
                    not_visited.append(cell)
            for i in range(len(not_visited_tmp)):
                if not unreachable(not_visited_tmp[i]):
                    not_visited.append(not_visited_tmp[i])

            # Calcul le point le plus proche, non visité

            to_visit = min(
                not_visited,
                key=lambda point: (point[0] - position[0]) ** 2
...
def unreachable(cell: Tuple[int, int]):
    ...
    for neighbour in getNeighbours(cell):
        # print(neighbour)
        if known_cells[neighbour] not in [
            None,
            HC.WALL,
            HC.GUARD_S,
            HC.GUARD_E,
...
def explorePaths(
    ...
    if current == end:
        print("*****")
        countMalus = compter_malus(path) + compter_rotations_et_mouvements(
        ...
        if len(paths) != 0 and countMalus >= min(paths, key=lambda x: x[1])[1]:
            ...
            paths.append(
                (list(path), countMalus)
            )  # Ajouter une copie du chemin à la liste des chemins
    else:
        ...
        for move in next_moves:
            ...
            # Vérifier si le mouvement est accessible
                ...
                known_cells[move]
                in [None, HC.WALL, HC.GUARD_S, HC.GUARD_E, HC.GUARD_N, HC.GUARD_W]
                or move in known_guards.keys()
            ):
                continue
            ...
            # On fait le choix de ne pas prendre les chemins deux fois plus long que le chemin de cout minimal déjà connu
                ...
                paths != []
                and path != []
                and compter_malus(path)
                + compter_rotations_et_mouvements(path, start_direction)
                > min(paths, key=lambda x: x[1])[1]
            ):
                continue
...
def compter_rotations_et_mouvements(chemin, orientation_initiale) -> int:
...
def detectGuards():
    ...
    for cell in [
        pos
        for pos in known_cells.keys()
        if (known_cells[pos] is None and pos not in known_guards.keys())
    ]:
...
def define_danger_zone(cell: Tuple[int, int]):
    global danger_zone
    c, r = cell[0], cell[1]
