...
def find_neighbour(
    cell: Tuple[int, int], orientation: Orientation
...
def turn_clockwise(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS : tourner à 90 °"""
    new_state: State = copy.deepcopy(state_t)
    orientation = state_t["look_at"]
    if orientation is not None:
        new_orientation = Orientation(state_t["succView"][orientation])
        new_state["look_at"] = new_orientation
        return (new_state, 1)
    return None


def turn_anticlockwise(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS : tourner à -90°"""

    new_state: State = copy.deepcopy(state_t)
    orientation = state_t["look_at"]
    if orientation is not None:
        new_orientation = Orientation(get_key(orientation, state_t["succView"]))
        new_state["look_at"] = new_orientation
        return (new_state, 1)
    return None


def move(state_t: State) -> Optional[Tuple[State, int]]:
    ...
    cost = 1
    if pos is not None:
        if (pos, ori) in state_t["proximity"].keys():
            new_pos = state_t["proximity"][(pos, ori)]
            if new_pos in state_t["empty"]:
...
def kill_target(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS :tuer la cible"""
    new_state: State = copy.deepcopy(state_t)
    pos = state_t["at"]
    if pos is not None and state_t["has_wire"] and pos == state_t["target"]:
        new_state["target_killed"] = True
        cost = 1
        if state_t["is_invisible"]:
            return (new_state, 0)
        else:
            # Malus si dans le champ de vision de quelqu'un, et sans costume
            for zone in state_t["guard_view"].values():
                cost += zone.count(pos) * 100
            for zone in state_t["civil_view"].values():
                cost += zone.count(pos) * 100
            return (new_state, cost)
    return None


def kill_guard(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS : tuer un garde"""
    new_state: State = copy.deepcopy(state_t)
    pos = state_t["at"]
    ori = state_t["look_at"]
    if pos is not None and ori is not None:
        if (pos, ori) in state_t["proximity"].keys() and (
            state_t["proximity"][(pos, ori)] in state_t["guard"].keys()
        ):
            to_kill = state_t["proximity"][(pos, ori)]
            # if (to_kill in state['guard'].keys()):
            if pos not in state_t["guard_view"][to_kill]:
                cost = 21
...
def kill_civil(state_t: State) -> Optional[Tuple[State, int]]:
    ...
    if pos is not None and ori is not None:
        if (pos, ori) in state_t["proximity"].keys() and (
            state_t["proximity"][(pos, ori)] in state_t["civil"].keys()
        ):
            to_kill = state_t["proximity"][(pos, ori)]

            # if (to_kill in state['civil'].keys()):
            if pos not in state_t["civil_view"][to_kill]:
                cost = 21
...
def grab_suit(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS : attraper le déguisement"""
    new_state: State = copy.deepcopy(state_t)
    pos = state_t["at"]
    if pos is not None and state_t["suit"] == pos:
...
def wear_suit(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS : porter le déguisement"""
    new_state: State = copy.deepcopy(state_t)
    pos = state_t["at"]
    if state_t["has_suit"] and not state_t["is_invisible"]:
...
def grab_wire(state_t: State) -> Optional[Tuple[State, int]]:
    """Action STRIPS : attraper la corde de piano"""
    new_state: State = copy.deepcopy(state_t)
    pos = state_t["at"]
    if pos is not None and state_t["wire"] == pos:
...
def astar(start: State, goal: str) -> Optional:
    ...
    while to_check:
        ...
        for action in actions:
            ...
            after_action_node = Node(
                action[0][0], after_action_cost, action[1], current_node
            )
            ...
            if already_in:
                ...
                # si le coût de la nouvelle action nous donne
                # un chemin moins couteux, on va le choisir
...
def next_actions(
    state_t: State,
) -> list[Union[tuple[tuple[State, int], Action], tuple[State, Action]]]:
    """Détermination des actions possibles à l'état state"""
    to_test = []

    new_state_w_cost = turn_clockwise(state_t)
    if new_state_w_cost is not None:
...
def launch_killing(status: State, hr: HitmanReferee, known_map) -> NoReturn:
    """Lancement de la phase 2, objectif : récupérer la corde, tuer la cible et fuir"""
    global state
    state = initial_state(
        known_map, status["position"], status["orientation"], status["m"], status["n"]
    )
    ...
    path_leave, cost_leave, _ = astar(tk_state, "leave")
